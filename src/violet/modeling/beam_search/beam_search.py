...
class BeamSearch(object):
    ...
    def apply(self, visual: utils.TensorOrSequence, out_size=1, return_probs=False, **kwargs):
        ...
        with self.model.statefulness(self.b_s):
            for t in range(self.max_len):
                visual, outputs,past = self.iter(t, visual, outputs,past,  return_probs, **kwargs)
    ...
    def iter(self, t: int, visual: utils.TensorOrSequence,  outputs,  past, return_probs, **kwargs):
        cur_beam_size = 1 if t == 0 else self.beam_size

        word_logprob,past = self.model.step(t, self.selected_words, visual, None, past,mode='feedback', **kwargs)
